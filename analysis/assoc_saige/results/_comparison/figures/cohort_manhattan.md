# Main scan across three nested sample sets — narrow, intermediate, full
...
LEFT — every analysed variant of the narrow_mainland main scan at its genomic position, y = -log10 P, on the shared axis; drawn without thinning and rasterised. The box is 4.08 x 1.77 in, the same box scan.main.png gives its Manhattan, so a peak has the same shape on both. RIGHT — the same P-values ranked against a uniform null, with the pointwise 95% concentration band from the order-statistic Beta(i, n-i+1) distribution and this cohort's lambda_GC. All three QQs carry one limit, for the same reason the Manhattans do. The header counts and lambda_GC come from this cohort's own 02.scan/scan_qc.tsv rather than being recomputed, so they agree with scan.main.png and cohort_compare.png exactly. Chromosome numbers are on the bottom row, which this row shares.

**(b) intermediate_mainland — Manhattan and QQ**

LEFT — every analysed variant of the intermediate_mainland main scan at its genomic position, y = -log10 P, on the shared axis; drawn without thinning and rasterised. The box is 4.08 x 1.77 in, the same box scan.main.png gives its Manhattan, so a peak has the same shape on both. RIGHT — the same P-values ranked against a uniform null, with the pointwise 95% concentration band from the order-statistic Beta(i, n-i+1) distribution and this cohort's lambda_GC. All three QQs carry one limit, for the same reason the Manhattans do. The header counts and lambda_GC come from this cohort's own 02.scan/scan_qc.tsv rather than being recomputed, so they agree with scan.main.png and cohort_compare.png exactly. Chromosome numbers are on the bottom row, which this row shares.

**(c) full_mainland — Manhattan and QQ**

LEFT — every analysed variant of the full_mainland main scan at its genomic position, y = -log10 P, on the shared axis; drawn without thinning and rasterised. The box is 4.08 x 1.77 in, the same box scan.main.png gives its Manhattan, so a peak has the same shape on both. RIGHT — the same P-values ranked against a uniform null, with the pointwise 95% concentration band from the order-statistic Beta(i, n-i+1) distribution and this cohort's lambda_GC. All three QQs carry one limit, for the same reason the Manhattans do. The header counts and lambda_GC come from this cohort's own 02.scan/scan_qc.tsv rather than being recomputed, so they agree with scan.main.png and cohort_compare.png exactly. Chromosome numbers are on this row, shared by all three.
...
| narrow_mainland: controls | 1,776 |
| intermediate_mainland: controls | 2,079 |
| full_mainland: controls | 2,662 |
| narrow_mainland: N_eff | 1,356 |
| intermediate_mainland: N_eff | 1,422 |
| full_mainland: N_eff | 1,507 |
| narrow_mainland: lambda_GC | 1.039 |
| intermediate_mainland: lambda_GC | 1.038 |
| full_mainland: lambda_GC | 1.021 |
| narrow_mainland: variants analysed | 5,113,626 |
| intermediate_mainland: variants analysed | 5,113,982 |
| full_mainland: variants analysed | 5,115,116 |
| narrow_mainland: excluded on ERRCODE / degenerate fit | 0 |
| intermediate_mainland: excluded on ERRCODE / degenerate fit | 0 |
| full_mainland: excluded on ERRCODE / degenerate fit | 0 |
| narrow_mainland: genome-wide peaks | 0 |
| intermediate_mainland: genome-wide peaks | 1 |
| full_mainland: genome-wide peaks | 0 |
| shared -log10 P ceiling | 8.4 |
| loci named (union, genome-wide in any cohort) | FTO |
...
|---|---|---|---|---|
| FTO | 16 | 53,882,967 | 3.836e-08 | intermediate_mainland |
...
2. A diamond means that cohort called the locus genome-wide. A guide with no diamond means the locus is there but below the threshold in that cohort — FTO in narrow_mainland is the case here, at P = 1.36e-07.
...
4. Read the header numbers together with the row, and the QQ beside it. lambda_GC spans 1.021-1.039 across the 3 sets; if that span is narrow and the QQ curves have the same shape, the calibration does not change as the filter is relaxed, so a column that grows down the figure is growing with N and not with inflation.
...
- Only the main model is drawn. Peaks called under the other genetic models are on their own scan figures and are absent here by construction.
- Suggestive peaks are not marked. The dashed line shows the tier; naming them across 3 rows would put 88 labels on the figure.
