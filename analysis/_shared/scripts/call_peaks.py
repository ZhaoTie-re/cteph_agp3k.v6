...
#             lead_variants.tsv  one lead per FOLLOWED-UP peak — the fan-out key
#             <peak>.sumstat.tsv per-peak statistics, for the followed-up peaks
#
#           peaks.tsv and lead_variants.tsv deliberately DIFFER in length.
#           peaks.tsv is the whole tier, because the scan figure's peak panel,
#           cross_cohort.py and the QC counts are all statements about the
#           scan. lead_variants.tsv is the fan-out key and is CAPPED: at 1e-5
#           the suggestive tier holds 40-51 peaks per cohort against ~51
#           expected by chance, so following up all of them is following up
#           noise. Do not 'fix' the inconsistency by making them match.
#
...
def parse_args():
    p = argparse.ArgumentParser(description="Scan QC and peak calling for one cohort.")
    ...
    p.add_argument('--peak-model', default='additive',
                   help='The ONLY model peaks are called from.')
    p.add_argument('--p-genomewide', type=float, default=5e-8)
    p.add_argument('--p-suggestive', type=float, default=1e-5)
    p.add_argument('--max-suggestive-followup', type=int, default=10,
                   help='How many suggestive peaks reach the per-peak fan-out, '
                        'smallest P first. Genome-wide peaks are never capped. '
                        'Negative disables the cap. MUST equal the scan figure\'s '
                        '--label-suggestive: see the invariant below.')
...
def model_order(scans, peak_model):
    """{model: rank} — the order models were supplied in, peak model first.

    The peak model leads because it is the one every table and figure is keyed
    on; the rest keep their input order so the caller controls the layout.
    """
    names = list(scans)
    if peak_model in names:
        names = [peak_model] + [m for m in names if m != peak_model]
...
def peaks_by_model(df, alpha_sug, alpha_gw, flank, cohort, model):
    ...
    Identical rule to the peak-model caller — called once at the suggestive
    threshold, then tiered by whether the lead clears 5e-8 — so a peak means the
    same thing in every model and the peak model's rows here reproduce peaks.tsv
...
def main():
    ...
    qc = pd.DataFrame(qc)
    # Canonical model order = the order the models were GIVEN, with the peak
    # model first. A hard-coded list of genetic-model names cannot serve a
    # component whose models are covariate sets rather than codings.
    qc['_o'] = qc['model'].map(model_order(scans, args.peak_model)).fillna(99)
    ...
    order = model_order(scans, args.peak_model)
    hit_rows = []
    for model in sorted(scans, key=lambda m: order.get(m, 99)):
    ...
    if len(sig):
        ...
        for i, (chrom, start, end, members) in enumerate(peaks, start=1):
            ...
            # THE PREFIX IS LOAD-BEARING. Both pipelines derive a peak's tier
            # from it (tierOf() in the .nf) to place per-peak outputs under
            # 03.peaks/<tier>/ and to pick the conditional-analysis threshold,
            # because a publishDir closure can only see process inputs. Changing
            # it here without changing tierOf() there fails loudly, by design.
    ...
    # ── the fan-out key: EVERY peak is computed; only some are drawn ────────
    # LD, fine-mapping and conditional analysis run for the WHOLE tier, because
    # the conditional result is what decides whether two nearby leads are one
    # locus or two (see cross_cohort.py). That judgement has to be available for
    # every peak or it would depend on whether the peak happened to be followed
    # up, which is circular.
    #
    # The cap now marks which peaks get FIGURES, and nothing else. `figure` = 1
    # for every genome-wide peak plus the N strongest suggestive ones.
    #
    # THE INVARIANT it preserves: N is the same number plot_manhattan_qq.py
    # labels with (--label-suggestive), so a suggestive locus NAMED on the scan
    # figure always has regional/fine-map/conditional FIGURES, and one that is
    # not named never does. Both pipelines pass params.MaxSuggestive to both
    # places; changing one without the other breaks the invariant silently.
    paired = list(zip(peak_rows, lead_rows))
    sug = sorted((pl for pl in paired if pl[1]['tier'] == TIER_SUG),
                 key=lambda pl: pl[0]['lead_p'])
    n_sug_total = len(sug)
    cap = args.max_suggestive_followup
    drawn = {pl[1]['peak_id'] for pl in paired if pl[1]['tier'] == TIER_GW}
    drawn |= {pl[1]['peak_id'] for pl in (sug[:cap] if cap >= 0 else sug)}
    for r in lead_rows:
        r['figure'] = int(r['peak_id'] in drawn)

    pd.DataFrame(lead_rows, columns=['cohort', 'peak_id', 'tier', 'lead_id', 'chrom',
                                     'pos', 'start', 'end', 'figure']
                 ).to_csv(out / 'lead_variants.tsv', sep='\t', index=False)

    # ── per-peak summary statistics, for the followed-up peaks ──────────────
    # Both tiers receive per-peak follow-up (LD, fine-mapping, conditional
    # analysis), and none of it can run without the window statistics.
    n_written = 0
    for r in lead_rows:
    ...
    print(f'[call_peaks] {args.cohort}: N={n_case}/{n_ctrl} Neff={n_eff:.1f}; '
          f'{len(peak_rows)} {args.peak_model} peaks ({n_gw} genome-wide, '
          f'{len(peak_rows) - n_gw} suggestive); ALL computed, '
          f'{n_drawn} drawn ({n_gw} genome-wide + '
          f'{n_drawn - n_gw} of {n_sug_total} suggestive'
          + (f', capped at {cap}' if cap >= 0 and n_sug_total > cap else '') + ')'
          + f'; {n_written} per-peak sumstat file(s); '
