...
def write_stats(png_path, *, peak, values):
    """Drop one locus's numbers beside its PNG, as `<png>.stats.json`.

    A fan-out figure does not write its own document: the prose would be
    identical for every locus and only these numbers would differ. The family
    catalogue (figure_catalogue.py) collects these files and renders them as one
    table, so a reader gets one document per family instead of one per locus.
    """
    out = Path(png_path).with_suffix('.stats.json')
    out.write_text(json.dumps({'peak': peak, 'values': [list(v) for v in values]}, indent=1))
    return out
...
def write_doc(png_path, *, title, question, panels, interpretation=None, numbers=None,
              ...
              methods_ref='../../../docs/METHODS.md', out_path=None, subject=None):
    """Write `<png>.md` next to `<png>`, or a standalone document at `out_path`.
    ...
    out_path : write here instead of beside a PNG. Used for a catalogue that
               documents a whole family of figures at once, where the prose is
               identical across them and only the numbers differ.
    subject  : replaces the `Figure file:` line — name the set of figures the
               document covers when it is not describing a single PNG.
    """
    if out_path is not None:
        out = Path(out_path)
        head = subject or f'**Covers:** {out.parent.name}'
    else:
        png = Path(png_path)
        out = png.with_suffix('.md')
        head = subject or f'**Figure file:** `{png.name}`'
    L = [f'# {title}', '', head, '',
