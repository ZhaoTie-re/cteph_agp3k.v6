...
def parse_args():
    ...
    p.add_argument('--p-genomewide', type=float, default=S.GW_ALPHA)
    p.add_argument('--p-suggestive', type=float, default=S.SUGGEST_ALPHA)
...
def main():
    ...
    for frame, name in ((gwh, 'model_peaks'), (leads, 'lead')):
        if not len(frame):
            continue
        bad = frame[((frame.tier == 'genome_wide') & (frame.P >= args.p_genomewide))
                    | ((frame.tier == 'suggestive')
                       & ((frame.P >= args.p_suggestive) | (frame.P < args.p_genomewide)))]
        if len(bad):
    ...
    print(f'[annotate_leads] {args.cohort}: {len(leads)} peak leads '
