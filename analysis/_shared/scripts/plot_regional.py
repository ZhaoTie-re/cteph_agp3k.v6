...
def draw_panel(ax, ss, ld, source, args, ymax, label_lead=False):
    ...
    if lead.any():
        ...
        # The lead is NAMED ONCE, in the figure header — it is the same variant in
        # all three panels, and the diamond is already a legend key. `label_lead`
        # survives because it also decides which panel gets the tier-line labels.

    S.tier_lines(ax, args.alpha, args.alpha_suggestive, label=label_lead)
    ax.set_ylim(0, ymax)
...
def main():
    ...
    all_counts = {}
    TITLE = dict(S.LD_SOURCE_TITLE, **dict(s.split('=', 1) for s in args.panel_label if '=' in s))
    ...
    S.caption_block(
        ...
        notes=(f'Window chr{str(chrom).replace("chr", "")}:{int(ss.POS.min()):,}\u2013'
               ...
               'recombination rate (right axis). Full explanation: ../README.md'),
        header=ident,
    ...
    figure_doc.write_stats(
        ...
        values=[('cohort', args.cohort), ('gene', ident[0] or '.'),
                ...
                ('window', f"chr{str(chrom).replace('chr','')}:"
                           f'{int(ss.POS.min()):,}-{int(ss.POS.max()):,}'),
