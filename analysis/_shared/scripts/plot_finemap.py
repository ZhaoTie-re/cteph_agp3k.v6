...
def main():
    ...
    S.tier_lines(ax, args.alpha, args.alpha_suggestive, label=True)
    ax.set_ylim(0, max(-np.log10(pip['P'].min()) * 1.16,
                       -np.log10(min(args.alpha, args.alpha_suggestive)) * 1.12))
    ...
    # fit above a 2.2 in panel. Anchored to the AXES, not to the canvas top: a
    # canvas-anchored legend does not move when caption_block grows the figure for
    # the identity header, and the two then print on top of each other.
    S.legend_above(ax_p, S.ld_legend_handles(include_unknown=True, lead=True),
                   title=f'in-sample {S.R2_LD} with the lead', ncol=7,
                   full_width=True, pad_in=0.30)
