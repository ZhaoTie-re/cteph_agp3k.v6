...
STAT_FIELDS = [
    'rsID', 'rsID_unresolved', 'Gene', 'Gene_Biotype', 'Gene_Distance_bp',
...
def lookup_rsids(tabix, vcf, variants, timeout=120):
    """{variant_id: (accessions, resolved)} from the configured rsID VCF.

    `variants` is an iterable of (variant_id, chrom, pos). The match is on the
    reconstructed chr:pos:REF:ALT key, never on position alone — position alone
    would assign a multi-allelic site's rsID to the wrong allele.

    Matching the right RECORD is not enough, because the source VCF is `norm`ed:
    splitting a multi-allelic record COPIES the whole ID field onto every split
    allele instead of distributing the accessions. The result is an ID string
    stamped on several different alleles at one position — but that is only a
    defect when the string holds MORE THAN ONE accession:

        chr2:232445059   rs790041                    C>T, C>G
            one accession, two alleles. RS numbers are site-level, so rs790041
            names this SNV whichever ALT we took. RESOLVED.

        chr12:114309388  rs75682226                  GGGCTT>G, G>GGGCTT
            one accession, the same indel written from two anchors. RESOLVED.

        chr10:1747167    rs1347066655;rs1564211184   CGTG>C, C>T
            two accessions on a 3 bp deletion and an SNV — the ID field is the
            UNION over the alleles of the pre-split record. At most one names
            each allele and the normalised file no longer says which. UNRESOLVED.

    So the key rule extends to the ACCESSION: several accessions are attributed
    to our variant only when no other allele at that position carries the same ID
    string. `resolved` is False otherwise, and the caller must not present those
    accessions as this variant's rsID. A genuine dbSNP merge — several accessions
    on ONE allele, shared with no other — stays resolved.
    """
    out = {}
    ...
    for vid, chrom, pos in variants:
        ...
        try:
            ...
            continue
        hit, alleles_per_id = None, {}
        for line in res.stdout.splitlines():
            f = line.split('\t')
            if len(f) < 5 or f[2] in ('.', ''):
                continue
            alleles_per_id.setdefault(f[2], set()).add((f[3], f[4]))
            if f'chr{_plain(f[0])}:{f[1]}:{f[3]}:{f[4]}' == vid:
                hit = f[2]
        if hit is not None:
            shared = len(alleles_per_id.get(hit, ())) > 1
            out[vid] = (hit, not (shared and ';' in hit))
    return out
...
def _rsid_of(rsids, vid):
    """The accession(s), ONLY when they are resolved to this variant's allele.

    Everything downstream that displays an rsID goes through here, so an
    un-distributed ID field can never be shown as a variant's name; the raw
    string stays available in `rsID_unresolved`.
    """
    hit = rsids.get(vid)
    return hit[0] if (hit is not None and hit[1]) else '.'


def _rsid_unresolved(rsids, vid):
    """The accession(s) the source VCF gave when they are NOT allele-resolved."""
    hit = rsids.get(vid)
    return hit[0] if (hit is not None and not hit[1]) else '.'
...
def annotate_variants(records, *, bfile, plink2, tabix, work, threads=1,
    ...
    for r in recs:
        ...
        row = {
            ...
            'rsID': _rsid_of(rsids, vid), 'rsID_unresolved': _rsid_unresolved(rsids, vid),
