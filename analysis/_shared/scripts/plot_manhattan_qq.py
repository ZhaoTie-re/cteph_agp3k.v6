...
def parse_args():
    ...
    p.add_argument('--peak-flank', type=int, default=250000,
                   help='Peak half-width, quoted in the caption. Must match call_peaks.py.')
...
# Columns the peak table is USED as numbers by — sorted on, plotted, compared to
# a threshold. Coerced explicitly rather than left to inference, because a scan
# that found NO peaks yields an empty column that pandas types as `object`, and
# every numeric method on it (nsmallest, min, comparison) then raises. A scan
# finding nothing is a legitimate outcome and must still draw its figure.
PEAK_NUMERIC = ('P', 'OR', 'L95', 'U95', 'MAF', 'A1_FREQ', 'pos',
                'n_sig_variants', 'n_genomewide_variants')
...
def read_peaks(path, model):
    ...
    d = d[d['model'] == model].copy() if 'model' in d else d
    for c in PEAK_NUMERIC:
        if c in d.columns:
...
def main():
    ...
    if lam < 0.9:
        calib = (f' This scan is deflated ({S.LAMBDA_GC} = {lam:.3f}): the recessive coding leaves '
                 ...
                 f'Its peaks are not comparable with a well-calibrated scan.')
    elif lam > 1.05:
    ...
    figure_doc.write_doc(
        ...
        interpretation=(f'{n_err:,} of {n_all:,} variants carried a non-null ERRCODE or a degenerate '
                        ...
                        f'findings; it receives no per-peak follow-up. Only the peak model '
                        f'defines the downstream fan-out — fine-mapping, conditional analysis and '
                        f'regional plots run on its peaks alone — so every other model is '
        ...
        panels=[
            ('a', 'Manhattan plot',
             ...
             f'P-values. Solid red line is genome-wide significance ({S.sci(args.alpha)}); dashed grey is '
             f'the suggestive threshold ({S.sci(args.suggestive)}). The x tick labels are on (b), which '
             'shares this axis.'),
            ('b', 'Peaks of this scan',
             'One marker per peak, at its lead variant, on the same x-axis as (a). Peaks are formed '
             f'by merging variants that pass P < {S.sci(args.suggestive)} and lie within '
             f'{args.peak_flank // 1000:,} kb; the lead is the '
        ...
        reading=[
            ...
            'a scan far from 1 in either direction is discussed in METHODS §7.',
            ...
            'which run on the peak model alone.',
