#!/usr/bin/env python3
# =============================================================================
# plot_mainland_pca_projection.py
# -----------------------------------------------------------------------------
# Purpose : Mainland BBJ PCA + cohort projection figures & case-vs-control statistics.
# Project : cteph_agp3k.v6 WGS pipeline (wgs.auto.par/select.auto.par.v6.nf)
# Used by : MAINLAND_PCA_PROJECTION
# =============================================================================
"""Mainland BBJ PCA space + cohort projection: publication figures and statistics.

The PCA space is built on BBJ *mainland* samples only; our cohort's *mainland*
samples are projected onto it. This script visualizes the merged projection and
computes case-vs-control statistics.

Outputs (all prefixed with --out-prefix):
1) .variance_summary.png        explained / cumulative variance from base eigenvalues.
2) .pc_pairs.pdf                 pairwise PC scatter (PC1-PC2, PC3-PC4, ...) with a grey
                                 BBJ-mainland background and cohort case/control overlaid.
3) .pc_group_distribution.png    per-PC case/control KDE distributions.
4) .pc_group_distribution.log.txt per-PC summary stats + case-vs-control tests.
5) .pc_stats.tsv / .pc_tests.tsv machine-readable versions of the above.

Group labels: BBJ samples by IID prefix (default 'bbj_'); cohort case/control via
sample_info phenotype column. Inferential tests and KDE use only the case & control
groups (BBJ is background context, excluded from the tests).

Reuses the design of plot_bbj_projection.py (viz) and plot_popgmm_pihat_projection.py
(statistics) so the outputs match the rest of the pipeline in style.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot mainland BBJ PCA variance/projection and case-vs-control statistics."
    )
    parser.add_argument("--bbj-eigenval", required=True, help="Path to BBJ (mainland) eigenval file")
...
def _sanitize_sample_info(sample_info: pd.DataFrame, sample_id_col: str) -> pd.DataFrame:
    """Drop rows with invalid sample IDs (NA/empty/'nan') in sample_id_col."""
...
def _build_group_descriptives(
    plot_df: pd.DataFrame,
    pc_cols: list[str],
    groups: list[str],
) -> pd.DataFrame:
    """Per-PC per-group mean/std/min/max/n for the requested groups (BBJ/case/ctrl)."""
    rows: list[dict[str, object]] = []
    for col in pc_cols:
        pc_num = int(str(col).replace("PC", ""))
        row: dict[str, object] = {"pc": f"PC{pc_num}"}
        for g in groups:
            x = pd.to_numeric(plot_df.loc[plot_df["GROUP"].eq(g), col], errors="coerce").to_numpy(
                dtype=float, copy=False
            )
            x = x[np.isfinite(x)]
            s = _safe_stats(x)
            row[f"n_{g}"] = int(x.size)
            row[f"{g}_mean"] = s["mean"]
            row[f"{g}_std"] = s["std"]
            row[f"{g}_min"] = s["min"]
            row[f"{g}_max"] = s["max"]
        rows.append(row)
...
def _build_pc_stats_and_tests(
    ...
    for col in pc_cols:
        ...
        if x_case.size >= 2 and x_ctrl.size >= 2:
            t_res = ttest_ind(x_case, x_ctrl, equal_var=False, nan_policy="omit")
            u_res = mannwhitneyu(x_case, x_ctrl, alternative="two-sided")
            t_stat, p_t = float(t_res[0]), float(t_res[1])
            u_stat, p_u = float(u_res[0]), float(u_res[1])
        else:
            t_stat = p_t = u_stat = p_u = np.nan
        test_rows.append(
            {"pc": f"PC{pc_num}", "t_stat": t_stat, "p_t": p_t, "u_stat": u_stat, "p_u": p_u}
        )
...
def _write_pc_distribution_log(
    *,
    ...
    logger = _setup_file_logger("plot_mainland_pca_projection_stats", log_path)

    logger.info("=" * 88)
    logger.info(f"{'MAINLAND PROJECTION ALL-PC DISTRIBUTION SUMMARY':^88}")
    logger.info("=" * 88)
    logger.info(f"  bbj_label             : {bbj_label}")
    logger.info(f"  control_label         : {ctrl_label}")
    logger.info(f"  case_label            : {case_label}")
    logger.info(f"  total_samples         : {len(plot_df):,}")
    logger.info(f"  bbj_samples           : {int((plot_df['GROUP'] == bbj_label).sum()):,}")
    ...
    logger.info("-" * 88)
    logger.info("Per-PC summary statistics (cohort case vs control):")
...
def plot_pairwise_scatter(
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(1, pc_count + 1, 2):
            ...
            size_map = {bbj_label: 4.0, ctrl_label: 14.0, case_label: 18.0, "OTHER": 8.0}
            alpha_map = {bbj_label: 0.60, ctrl_label: 0.80, case_label: 0.90, "OTHER": 0.60}
            zorder_map = {bbj_label: 1, ctrl_label: 2, case_label: 3, "OTHER": 2}
            ...
            for group in order:
                ...
                ax.scatter(
                    sub[x_col],
                    sub[y_col],
                    s=size_map.get(group, 8.0),
                    alpha=alpha_map.get(group, 0.6),
                    c=color_map[group],
                    label=group,
                    edgecolors="none",
...
def plot_grouped_pc_kde(
    ...
    fig.suptitle(
        "Cohort case/control distributions on mainland BBJ PC axes",
        fontweight="bold",
        y=0.995,
    )
    fig.legend(
...
def main() -> None:
    ...
    # BBJ labels are derived from IID prefix; cohort case/control from sample_info.
    ...
    # Statistics: cohort case vs control (BBJ excluded from inferential tests).
    ...
    group_desc = _build_group_descriptives(
        plot_df=plot_df,
        pc_cols=pc_cols,
        groups=[args.bbj_label, args.ctrl_label, args.case_label],
    )
    ...
    stats_tsv = Path(f"{out_prefix}.pc_stats.tsv")
    tests_tsv = Path(f"{out_prefix}.pc_tests.tsv")
    group_desc.to_csv(stats_tsv, sep="\t", index=False, float_format="%.6f")
    tests_df.to_csv(tests_tsv, sep="\t", index=False, float_format="%.6g")
    ...
    print(f"[OK] Group distribution log: {log_path}")
    print(f"[OK] Per-group descriptives TSV: {stats_tsv}")
    print(f"[OK] Case-vs-control tests TSV: {tests_tsv}")
